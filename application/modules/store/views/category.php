    <section class="uk-section uk-section-small uk-padding-remove slider-section">
    </section>
      ...
      <div class="uk-container">
        <div class="uk-grid uk-grid-medium" data-uk-grid>
          ...
          <div class="uk-width-2-3@m">
              <h5 class="uk-h4 uk-text-bold uk-margin-remove-bottom"><i class="fas fa-tag"></i> <?= $this->store_model->getCategoryName($route); ?></h5>
              ...
              <div class="uk-grid uk-grid-small uk-child-width-1-1 uk-child-width-1-2@s uk-child-width-1-3@m" data-uk-grid>
                ...
                <img src="<?= base_url();?>/assets/images/store/<?= $items->icon ?>.jpg" width="432" height="360" alt="icon">
				</div>
                      <!-- START CODE TO TOOLTIPS WOWHEAD -->
                      <?php if($items->type == 1): ?>
                      <span class="uk-text-middle">
                        <!-- You can use 'es.wowhead' or any other as 'ru, fr, cn' -->
                        <a <?= $items->command ?>"><?= $items->name ?></a>
                      </span>
                      <?php else: ?>
                      <span class="uk-text-middle">
                        <a href="#"><?= $items->name ?></a>
            ...
            <?php if ($items->dp > 0 && ($items->vp == 0 || $items->dp >= $items->vp)): ?>
            <span uk-tooltip="title:<?=$this->lang->line('panel_dp'); ?>">
            <i class="dp-icon"></i>
            </span> <?= $items->dp ?>
            <?php elseif ($items->vp > 0): ?>
