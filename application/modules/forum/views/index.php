    ...
    <div class="uk-grid uk-grid-medium uk-margin-small" data-uk-grid>
      <div class="uk-width-3-4@m">
        <?php foreach ($this->forum_model->getCategory() as $categorys) : ?>
          <div class="uk-overflow-auto uk-margin-medium forum-table uk-table-forum">
            <table class="uk-table uk-table-hover uk-table-middle">
              <caption uk-toggle="target: #cat-<?= $categorys->id ?>;animation: uk-animation-fade">
                <?= $categorys->name ?>
              </caption>
              <tbody id="cat-<?= $categorys->id ?>">
                <?php foreach ($this->forum_model->getCategory($categorys->id) as
                  $sections) : ?>
                  <?php if ($sections->type == 1 || $sections->type == 3) : ?>
                    <tr>
                      <td class="uk-table-shrink">
                        <i class="forum-icon" style="
                      background-image: url('<?= base_url('assets/images/forums/' . $sections->icon); ?>');
                    "></i>
                      </td>
                      ...
                      <td class="uk-width-small uk-text-center">
                        <span class="uk-display-block uk-text-bold"><i class="far fa-file-alt"></i>
                          <?= $this->forum_model->getCountTopics(
                            'forums',
                            $sections->id
                          ) ?></span>
                      ...
                      <td class="uk-width-medium">
                        <?php foreach ($this->forum_model->getLastPosts($sections->id)
                          as $lastpost) : ?>
                          ...
                          by
                          <span class="uk-text-primary"><?= $this->wowauth->getUsernameID($lastpost->author)
                                                        ?></span>
                        <?php endforeach; ?>
                      </td>
                    </tr>
                  <?php elseif ($sections->type == 2) : ?>
                    <?php if ($this->wowauth->isLogged()) : ?>
                      <?php if (
                        $this->wowauth->getRank($this->session->userdata('wow_sess_id'))
                        > 0
                      ) : ?>
                        <tr>
                          <td class="uk-table-shrink">
                    ...
                    "></i>
                          ...
                          <td class="uk-width-small uk-text-center">
                            <span class="uk-display-block uk-text-bold"><?= $this->forum_model->getCountTopics(
                                                                          'forums',
                                                                          $sections->id
                                                                        ) ?></span>
                            <span class="uk-text-small"><?= $this->lang->line('forum_posts_count'); ?></span>
        ...
        <div class="uk-card uk-card-forum">
          <div class="uk-card-header">
            <h3 class="uk-card-title uk-text-center">
              <?= $this->lang->line('forum_last_activity'); ?>
            </h3>
          </div>
          <img class="uk-divider" src="<?= $template['location'] . 'assets/images/separator.png' ?>" />
            ...
            <ul class="uk-list uk-list-divider">
              <?php foreach ($this->forum_model->getLastPosts() as $lastest) : ?>
                <li>
                  ...
                  <?php if (
                    $this->forum_model->getLastReplies($lastest->id)->num_rows()
                    == 0
                  ) : ?>
                    <p class="uk-text-small uk-margin-remove">
                      <?= $this->lang->line('forum_last_post_by'); ?>
                      <span class="uk-text-primary"><?= $this->wowauth->getUsernameID($lastest->author)
                                                    ?></span>
                    </p>
                    <p class="uk-text-small uk-margin-remove">
                      <?= date('d-m-y h:i:s', $lastest->date) ?>
                    </p>
                  <?php else : ?>
                    <?php foreach ($this->forum_model->getLastReplies($lastest->id)->result()
                      as $replies) : ?>
                      <p class="uk-text-small uk-margin-remove">
                        <?= $this->lang->line('forum_last_post_by'); ?>
                        <span class="uk-text-primary"><?= $this->wowauth->getUsernameID($replies->author)
                                                      ?></span>
                      </p>
                      <p class="uk-text-small uk-margin-remove">
                        <?= date('d-m-y h:i:s', $replies->date) ?>
                      </p>
                    <?php endforeach; ?>
                  <?php endif; ?>
                </li>
    ...
    <div class="uk-card uk-card-forum uk-margin-small">
      <div class="uk-card-header">
        <h3 class="uk-card-title">
          ...
          <?= $this->lang->line('forum_whos_online'); ?>
        </h3>
      </div>
      <div class="uk-card-body">
        <p class="uk-margin-remove">
          0 users active in the past 15 minutes (0 members, 0 of whom are
          invisible, and 0 guests).
        </p>
        <hr class="uk-hr uk-margin" />
        <div class="uk-grid uk-grid-medium uk-child-width-auto uk-flex-center" data-uk-grid>
          <div>
            <div class="forum-who-icon">
